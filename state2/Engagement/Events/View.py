...
def insert_bot_view_data(tag):
    ...
    with engine.connect() as conn:
        ...
        for mod_value in range(100):  # ✅ 改为100分片
...
def overwrite_bot_view_table_with_summary(tag):
    ...
    SELECT 
        ...
        ROUND(SUM(total_bot_view) / SUM(unique_bot_view_users), 4) AS bot_view_ratio,
    ...
    with engine.connect() as conn:
        ...
        for _, row in summary_df.iterrows():
            insert_query = f"""
            INSERT INTO {table_name} (variation, total_bot_view, unique_bot_view_users, bot_view_ratio, experiment_name)
            VALUES ('{row['variation']}', {row['total_bot_view']}, {row['unique_bot_view_users']}, {row['bot_view_ratio']}, '{row['experiment_name']}');
            """
