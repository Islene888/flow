...
def insert_chat_data(tag):
    ...
    with engine.connect() as conn:
        ...
        for mod_value in range(100):
            ...
            INSERT INTO {table_name} (variation, total_chat, total_chat_users, chat_per_user, experiment_name)
            ...
            WHERE a.experiment_id = '{experiment_name}'
              AND FROM_UNIXTIME(FLOOR(c.ingest_timestamp / 1000)) BETWEEN '{start_time}' AND '{end_time}'
    ...
    print(f"所有Chat批次数据插入完成，目标表：{table_name}")
    return table_name


# ============= 计算汇总并覆盖原表 =============
def overwrite_chat_table_with_summary(tag):
    print(f"开始生成Chat事件汇总数据，并覆盖到原表，标签：{tag}")
    ...
    with engine.connect() as conn:
        ...
        for _, row in summary_df.iterrows():
            insert_query = f"""
            INSERT INTO {table_name} (variation, total_chat, total_chat_users, chat_per_user, experiment_name)
            VALUES ('{row['variation']}', {row['total_chat']}, {row['total_chat_users']}, {row['chat_per_user']}, '{row['experiment_name']}');
            """
            conn.execute(text(insert_query))

    print(f"Chat事件汇总数据已覆盖表：{table_name}")


# ============= 主流程 =============
def main(tag):
    print("主流程开始执行 - Chat事件")

    table_name = insert_chat_data(tag)
    if table_name is None:
        print("Chat事件数据写入或建表失败！")
        return

    overwrite_chat_table_with_summary(tag)

    print("主流程执行完毕 - Chat事件")
