    ...
    with engine.connect() as conn:
        ...
        for mod_value in range(100):  # 改为100分片
            ...
            SELECT /*+ SET_VAR(query_timeout = 30000) */
                ...
                ROUND(COUNT(DISTINCT c.event_id) * 1.0 / COUNT(DISTINCT c.user_id), 4) AS edit_ratio,
            ...
            WHERE a.experiment_id = '{experiment_name}'
              ...
              AND c.method = 'edit'
              AND MOD(crc32(c.user_id), 100) = {mod_value}  -- 分片100
...
def overwrite_edit_table_with_summary(tag):
    ...
    SELECT 
        ...
        ROUND(SUM(total_edit) / SUM(unique_edit_users), 4) AS edit_ratio,
