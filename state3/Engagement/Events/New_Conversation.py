    ...
    with engine.connect() as conn:
        ...
        for d in range(1, delta_days):  # 跳过首日和末日
            ...
            INSERT INTO {table_name} (
                event_date, variation, total_new_conversation, unique_new_conversation_users, new_conversation_ratio, experiment_name
            )
            SELECT
            ...
            WHERE b.experiment_id = '{experiment_name}'
              ...
              AND a.event_date BETWEEN '{start_day_str}' AND '{end_day_str}'
            GROUP BY b.variation_id;
            """
            print(f"👉 正在插入日期：{current_date}")
            conn.execute(text(insert_query))

        print(f"✅ 所有 daily new_conversation 数据已成功写入 {table_name}。")
